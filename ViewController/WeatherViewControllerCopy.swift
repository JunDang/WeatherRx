...
class WeatherViewController: UIViewController {
    ...
    var flickrViewModel2: FlickrViewModel2?
    var flickrViewModelObservable: Observable<FlickrViewModel?>?
    var backgroundImageObservable: Observable<UIImage?>?
    var backgroundImage = BehaviorRelay<UIImage?>(value: nil)
    let locationManager = CLLocationManager()
    var searchTextField: UITextField?
    var dataModelObservable: Observable<DataModel>?
    override func viewDidLoad() {
        ...
        print("locationDriver: " + "\(locationDriver)")
        ...
        weatherForecastData = locationDriver.asObservable()
            .flatMap(){ location -> Observable<(AnyRealmCollection<WeatherForecastModel>, RealmChangeset?)> in
                ...
                print("latfromgeo: " + "\(lat)")
                let lon = location.longitude
                self.weatherViewModel = WeatherViewModel(lat: lat, lon: lon, apiType: InternetService.self)
                self.weatherForecastData = self.weatherViewModel?.weatherForecastData
                return self.weatherForecastData!
        }
        backgroundImageObservable = locationDriver.asObservable()
            .flatMap(){location -> Observable<UIImage?> in
                let lat = location.latitude
                let lon = location.longitude
                self.flickrViewModel2 = FlickrViewModel2(lat: lat, lon: lon, apiType: InternetService.self, imageDataCacheType: ImageDataCaching.self)
                self.backgroundImage = (self.flickrViewModel2?.backgroundImage)!
                return self.backgroundImage.asObservable()
        }
        dataModelObservable = readInDataModel(weatherForecastDataObservable: weatherForecastData!, flickrImageObservable: backgroundImageObservable!)
        
        
    }
    
    func readInDataModel(weatherForecastDataObservable: Observable<(AnyRealmCollection<WeatherForecastModel>, RealmChangeset?)>, flickrImageObservable: Observable<UIImage?>) -> Observable<DataModel> {
        
        let weatherForecastModelObservable = weatherForecastDataObservable
            .skip(1)
            .map(){weatherForecastData -> WeatherForecastModel in
        ...
        return Observable
            .zip(weatherForecastModelObservable, flickrImageObservable){(weatherForecastModel, flickrImage) in
                return DataModel(weatherForecastModel: weatherForecastModel, flickrImage: flickrImage!) }
        
    }
    /* flickrViewModelObservable = locationDriver.asObservable()
     .flatMap(){ location -> Observable<(AnyRealmCollection<WeatherForecastModel>, RealmChangeset?)> in
     print("map function called")
     let lat = location.latitude
     print("latfromgeo: " + "\(lat)")
     ...
     .flatMap(){weatherForecastData -> Observable<FlickrViewModel?> in
     let weatherForecastModel = weatherForecastData.0.first
     if weatherForecastModel != nil {
     let lat = weatherForecastModel!.latitude
     let lon = weatherForecastModel!.longitude
     //let currentWeather = weatherForecastModel!.currently!.summary
     let currentWeather = "sunny"
     //print("currentWeatherSummary: " + "\(currentWeather)")
     self.flickrViewModel = FlickrViewModel(lat: lat, lon: lon, currentWeather: currentWeather, apiType: InternetService.self, imageDataCacheType: ImageDataCaching.self)
     }/* else {
     self.flickrViewModel =  FlickrViewModel(lat: 43.6532, lon: -79.3832, currentWeather: "sunny", apiType: InternetService.self, imageDataCacheType: ImageDataCaching.self)
     }*/
     return Observable.just(self.flickrViewModel)
     }*/
    
    
    
    /*func getFlickrViewModelObservable(weatherForecastData: Observable<(AnyRealmCollection<WeatherForecastModel>, RealmChangeset?)>) -> Observable<FlickrViewModel> {
     
     }*/
    /*backgroundImageObservable = weatherForecastData?
     .flatMap(){weatherForecastData -> Observable<UIImage?> in
     let weatherForecastModel = weatherForecastData.0.first
     if weatherForecastModel != nil {
     let lat = weatherForecastModel!.latitude
     let lon = weatherForecastModel!.longitude
     let currentWeather = weatherForecastModel!.currently!.summary
     print("currentWeatherSummary: " + "\(currentWeather)")
     self.flickrViewModel2 = FlickrViewModel2(lat: lat, lon: lon, currentWeather: currentWeather, apiType: InternetService.self, imageDataCacheType: ImageDataCaching.self)
     self.backgroundImageObservable = self.flickrViewModel2?.backgroundImage
     }
     return (self.backgroundImageObservable)!
     }*/
    //bindBackground(backgroundImage: backgroundImageObservable!)
    
    
    // bindBackground(backgroundImage: Observable<UIImage?>)
    /* weatherForecastData?
     ...
     print("currentWeatherSummary: " + "\(currentWeather)")
     //self.flickrViewModel = FlickrViewModel(lat: lat, lon: lon, currentWeather: currentWeather, apiType: InternetService.self, imageDataCacheType: ImageDataCaching.self)
     //self.backgroundImage = (self.flickrViewModel?.backgroundImage)!
     //self.bindBackground(backgroundImage: self.backgroundImage)
     self.bindBackground(lat: lat, lon: lon, currentWeather: currentWeather)
     }
     })
     .disposed(by: bag)*/
    
    /*weatherViewModelObservable = locationDriver.asObservable()
     .map({ location in
     print("map function called")
     let lat = location.latitude
     print("latfromgeo: " + "\(lat)")
     let lon = location.longitude
     return WeatherViewModel(lat: lat, lon: lon, apiType: InternetService.self)
     })*/
    
    /* weatherViewModelObservable?.subscribe({element in
     print("weatherViewModelObservable: " + "\(element)")
     })*/
    ...
    /*func bindBackground(flickrViewModel: FlickrViewModel) {
     //let flickrViewModel: FlickrViewModel = FlickrViewModel(lat: lat, lon: lon, currentWeather: currentWeather, apiType: InternetService.self, imageDataCacheType: ImageDataCaching.self)
     //flickrViewModel = FlickrViewModel(lat: lat, lon: lon, currentWeather: currentWeather, apiType: InternetService.self, imageDataCacheType: ImageDataCaching.self)
    ...
    /* func bindBackground(backgroundImage: BehaviorRelay<UIImage?>) {
     //flickrViewModel = FlickrViewModel(lat: lat, lon: lon, currentWeather: currentWeather, apiType: InternetService.self, imageDataCacheType: ImageDataCaching.self)
     weatherForecastData?
     .flatMap(){weatherForecastData -> BehaviorRelay<UIImage?> in
     let weatherForecastModel = weatherForecastData.0.first
     if weatherForecastModel != nil {
     let lat = weatherForecastModel!.latitude
     let lon = weatherForecastModel!.longitude
    ...
    /*func bindBackground(backgroundImage: Observable<UIImage?>) {
     backgroundImage.asDriver(onErrorJustReturn: UIImage(named: "banff")!)
     .drive(onNext: { [weak self] backgroundImage in
     let resizedImage = backgroundImage?.scaled(CGSize(width: (self?.view.frame.width)!, height: (self?.view.frame.height)! * 1.5))
     self?.backgroundView.image = resizedImage
     self?.blurredImageView = DynamicBlurView(frame: (self?.view.bounds)!)
    ...
    /*override func viewDidAppear(_ animated: Bool) {
     let locationDriver = GeoLocationService.instance.getLocation()
     locationDriver.asObservable()
     .flatMap(){location -> BehaviorRelay<UIImage?> in
     let lat = location.latitude
     let lon = location.longitude
     self.flickrViewModel2 = FlickrViewModel2(lat: lat, lon: lon, apiType: InternetService.self, imageDataCacheType: ImageDataCaching.self)
     self.backgroundImage = (self.flickrViewModel2?.backgroundImage)!
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        dataModelObservable?
            .subscribe(onNext: { (dataModel) in
                
                self.currentWeatherView.update(with: dataModel.weatherForecastModel)
                self.tableViewController.weatherForecastModel = dataModel.weatherForecastModel
                self.tableViewController.tableView.reloadData()
                let resizedImage = dataModel.flickrImage.scaled(CGSize(width: (self.view.frame.width), height: (self.view.frame.height) * 1.5))
                self.backgroundView.image = resizedImage
                self.blurredImageView = DynamicBlurView(frame: (self.view.bounds))
                self.blurredImageView.blurRadius = 10
                self.blurredImageView.alpha = 0
        ...
        /* flickrViewModelObservable?
         .subscribe(onNext: { flickrViewModel in
         if flickrViewModel != nil {
         self.bindBackground(flickrViewModel: flickrViewModel!)
         }
         })
         .disposed(by: bag)*/
        /* backgroundImageObservable?
         .subscribe(onNext: { (backgroundImage) in
         self.bindBackground(backgroundImage:backgroundImage)
         })
         .disposed(by: bag)*/
    }
